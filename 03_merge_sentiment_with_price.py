...
# Convert timestamp to just the date (drop time)
df_news["date"] = df_news["date"].dt.date
df_news["date"] = pd.to_datetime(df_news["date"])  # back to datetime64 for merge

# Aggregate sentiment values per day
df_sentiment = df_news.groupby("date").agg({
    "sentiment_polarity": "mean",
    "sentiment_pos": "mean",
    "sentiment_neg": "mean",
    "sentiment_neu": "mean",
    "title": "count"  # proxy for number of articles
}).rename(columns={
    "title": "article_count",
    "date": "Date"
}).reset_index()
...
# Optionally fill missing sentiment values (e.g., days with no articles)
df_merged[[
